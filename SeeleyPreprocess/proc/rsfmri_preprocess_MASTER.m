...
realign=true; % do motion correction
slicetime=true; % do slice timing correction
...
bptfilter_nuisance=true; % apply same bandpass temporal filter to nuisance parameters (motion + ICA noise components)
motion_report=true; % calculate motion parameters; create dummy nuisance regressor for each TR with a motion spike
...
if gui
    ...
    motion_report=subjlist.opt.mspike; % create dummy nuisance regressor for each TR with a motion spike
...
if complete_run
    ...
    dir_suffix='TRCNnSFmDI'; % if not complete_run, have to manually specify suffix
end
...
if realign
    ...
    % input slice time corrected images
    for n = 1:n_scans
    ...
    addstr='ua';
    % create merged 4d functional file from [slice-time corrected, realigned] files for running through ICA
    merge='';
    for n = 1:n_scans
        [pth,nam,ext] = fileparts(subjlist.fname{n,:});
        merge = sprintf('%s %s',merge,strcat(interfmri_imgs_dir_fullpth,'/',addstr,nam,ext));
    end
    
    merged_file=sprintf('%s/%s/%s_merged',rspth,interfmri_imgs_dir,base_filename);
    ...
    cmd=sprintf('fslmaths %s -Tmean %s/mean_func_unsmoothed',merged_file,interfmri_dir_fullpth);
    fsl_matlab(cmd,false);
end

%% 4. ICA denoising
% run ICA in order to 1) generate Melodic ICA report for data quality diagnostics and 2) optionally create nuisance regressors for interleave components
if ica
    if ~ica_already_run
        melodic_design_template=sprintf('%s/melodic_design_template.fsf',files_dir)
        melodic_design_file='melodic_design.fsf';
        melodic_file=merged_file;
        ...
        cmd=sprintf('sed "s|/data6/mridata6/Hillblom/2679/2679_20100921/rsfmri/orig2/RestingState25x25x3_merged|%s|g" %s > melodic_design_temp1',melodic_file,melodic_design_template);
        unix(cmd);
        ...
        unix(cmd);
        % MNI image
        cmd=sprintf('sed "s|/usr/share/fsl/data/standard/MNI152_T1_2mm_brain|%s|g" melodic_design_temp5 > melodic_design_temp6',mni_brain);
        unix(cmd);
        
        % total number of voxels in all volumes combined
        cmd=sprintf('fslstats %s -v',melodic_file);
        [status,n_vox_str]=unix(cmd);
        n_vox_str_split=regexp(n_vox_str,' ','split');
        n_vox=n_vox_str_split{1};
        cmd=sprintf('sed "s|set fmri(totalVoxels) 71605440|set fmri(totalVoxels) %s|g" melodic_design_temp6 > %s',n_vox,melodic_design_file);
        unix(cmd);
        ...
        if setup_ica
            ...
            % generate unsmoothed mean functional image for spm coregistration
            cmd=sprintf('fslmaths %s -Tmean %s/mean_func_unsmoothed',melodic_file,melodic_dir);
            fsl_matlab(cmd,false);
        end
    end
end

%% 5. Motion spike detection
if motion_report
    % generate motion report and save motion statistics
    motion_dir='motion_corr';
    cmd=sprintf('mkdir %s/%s',interfmri_dir_fullpth,motion_dir);
    unix(cmd);
    
    % copy motion parameters to motion_report directory
    [pth,img1_nam,ext] = fileparts(subjlist.fname{1,:});
    motion_par_file=sprintf('%s/rp_a%s.txt',interfmri_imgs_dir_fullpth,img1_nam);
    cmd=sprintf('cp %s %s/%s',motion_par_file,interfmri_dir_fullpth,motion_dir);
    unix(cmd);
    ...
    [mean_disp,std_disp,max_disp,sum_disp,n_spikes,mean_transrms,std_transrms,max_transrms,sum_transrms,max_trans,mean_euler,std_euler,max_euler,sum_euler,max_rot,spike_trs]=motion_qa(motion_par_file,false,false,n_scans,spike_cutoff,motion_report_img);
    save(motion_stats_file,'mean_disp','std_disp','max_disp','sum_disp','n_spikes','mean_transrms','std_transrms','max_transrms','sum_transrms','max_trans','mean_euler','std_euler','max_euler','sum_euler','max_rot','spike_trs');
    
    n_bad=length(spike_trs);
    spike_regressors=zeros(n_scans,n_bad);
    for i=1:n_bad
        spike_regressors(spike_trs(i),i)=1;
    end
    
    spike_regressors_file=sprintf('%s/%s/spike_regressors.txt',interfmri_dir_fullpth,motion_dir);
    save(spike_regressors_file,'spike_regressors','-ascii');
end

%% 6a. FSL coregistration of fMRI to T1 + normalization of T1 to standard
...
%% 6b. SPM coregistration of fMRI to T1 + normalization of T1 to standard
...
if spm_reg
    if coreg
        ...
        if (length(subjlist.tname{1})~=0)
            ...
            if exist(strcat(interfmri_imgs_dir_fullpth,'/','mean',addstr,nam,ext))
                mean_func = strcat(interfmri_imgs_dir_fullpth,'/','mean',addstr,nam,ext); % mean func is created during realignment and only has 'ua' prefix
            elseif exist(strcat(interfmri_dir_fullpth,'/',melodic_dir,'/','mean_func_unsmoothed.nii')) ||  exist(strcat(interfmri_dir_fullpth,'/',melodic_dir,'/','mean_func_unsmoothed.nii.gz'))
            ...
            % (update [slice time corrected, realigned] files with co-registered equivalents)
    ...
    if norm
        if new_segment
            ...
            matlabbatch{1}.spm.util.defs.comp{1,2}.id.space={sprintf('%s/T1.nii',files_dir)};
            ...
            matlabbatch{1}.spm.spatial.normalise.estwrite.eoptions.tpm={[fileparts(which('spm')),'/tpm/TPM.nii']};
...
if bptfilter
    ...
    % if fslmaths bandpass filter demeans each voxel (after fsl version > 5.0.7), calculate the voxelwise mean and add it back to the filtered data
    filter_remean=true;
    if filter_remean
        demean_cutoff=1; % mean BOLD intensity value beneath which it is considered that fslmaths has demeaned the image
    ...
    % delete 4D files
    cmd=sprintf('rm %s/%s_merged.nii %s/%s_merged_filtered.nii',interfmri_imgs_dir_fullpth,base_filename,interfmri_imgs_dir_fullpth,addstr);
    unix(cmd);
end
...
if bptfilter_nuisance
    ...
    % get motion parameters from 1) SPM realignment rp_*.txt file, 2) FSL melodic.ica/mc dir, or 3) mcflirt params from motion spike detection
    if exist(sprintf('%s/%s/rp_a%s.txt',interfmri_dir_fullpth,motion_dir,img1_nam))
        [pth,img1_nam,ext] = fileparts(subjlist.fname{1,:});
        motion_file_fullpth=sprintf('%s/%s/rp_a%s.txt',interfmri_dir_fullpth,motion_dir,img1_nam);
        motion_file=dir(motion_file_fullpth);
    elseif exist(sprintf('%s/%s/mc/prefiltered_func_data_mcf.par',interfmri_dir_fullpth,melodic_dir))
