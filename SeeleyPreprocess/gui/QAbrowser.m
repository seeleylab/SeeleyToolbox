function varargout = QAbrowser(varargin)
% QABROWSER MATLAB code for QAbrowser.fig
%      QABROWSER, by itself, creates a new QABROWSER or raises the existing
%      singleton*.
%
%      H = QABROWSER returns the handle to a new QABROWSER or the handle to
%      the existing singleton*.
%
%      QABROWSER('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in QABROWSER.M with the given input arguments.
%
%      QABROWSER('Property','Value',...) creates a new QABROWSER or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before QAbrowser_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to QAbrowser_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help QAbrowser

% Last Modified by GUIDE v2.5 30-Jul-2014 15:32:52

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @QAbrowser_OpeningFcn, ...
    'gui_OutputFcn',  @QAbrowser_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before QAbrowser is made visible.
function QAbrowser_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to QAbrowser (see VARARGIN)
ROOT_DIR=regexprep(which('QAbrowser'),'gui/QAbrowser.m','proc/');
...
if ~isempty(procfiles)
   ...
   handles.imgFilePrefix='swua'; %regexprep(exampProc,'_.*','');
else
...
set(handles.registerFig,'name',[num2str(handles.index),'):',handles.subjdir{handles.index}],'numbertitle','off') 
...
set(handles.motionFig,'name',[num2str(handles.index),'):',handles.subjdir{handles.index}],'numbertitle','off') 
...
%Choose default command line output for QAbrowser
...
% UIWAIT makes QAbrowser wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = QAbrowser_OutputFcn(hObject, eventdata, handles)
...
if handles.index>=2
    ...
    guidata(hObject, handles);
    fslRegFile=[handles.subjdir{handles.index},'/interfmri_',preprocsuffix,'/example_func2standard.png'];
    if ishandle(handles.registerFig) 
    close(handles.registerFig); 
    end
    handles.registerFig=figure;
    set(handles.registerFig,'name',[num2str(handles.index),'):',handles.subjdir{handles.index}],'numbertitle','off') 
    [I,map]=imread(fslRegFile,'png');
    ...
    motionFigFile=[handles.subjdir{handles.index},'/interfmri_',preprocsuffix,'/motion_corr/motion_report.png'];
    if ishandle(handles.motionFig)
    close(handles.motionFig); 
    end
    handles.motionFig=figure;
    set(handles.motionFig,'name',[num2str(handles.index),'):',handles.subjdir{handles.index}],'numbertitle','off') 
    movegui(handles.motionFig,'northeast') 
