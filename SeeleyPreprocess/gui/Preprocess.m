...
% Last Modified by GUIDE v2.5 16-Jul-2015 22:41:23
...
if  strcmp(get(handles.ready,'String'),'SCAN')
    ...
    for x=1:length(handles.subjdir)
        ...
        processedfmriPath=regexprep(outdirRoot,'interfmri','processedfmri');
        if exist(outdir) && ~exist([processedfmriPath,'/images/swua_filteredf_0001.nii']) 
        ...
        elseif ~exist(outdir)
            ...
            unix(cmd)
            save([outdir,'preprocSetup.mat'],'subjlist');
        elseif ~exist(outdir) && exist([processedfmriPath,'/images/swua_filteredf_0001.nii'])
            unix(['rm -r ', processedfmriPath])
            cmd=['mkdir -p ', outdir];
            unix(cmd)
            save([outdir,'preprocSetup.mat'],'subjlist');
        end
